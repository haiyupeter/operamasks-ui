...
test("border", function() {
	...
	el = $("#tab-option-default").omTabs();
	hasBorder(el , '默认初始化时页签正文区是有边框的');
	
	el = $("#tab-option-border-true").omTabs({border:true});
	hasBorder(el , '初始化指定[border:true]时页签正文区是有边框的');
	
	el = $("#tab-option-border-false").omTabs({border:false});
...
test("closable", function() {
	...
	el = $("#tab-option-closable-false").omTabs({closable:false});
	...
	el = $("#tab-option-closable-array").omTabs({closable:[0,2]});
...
test("height", function() {
	...
	equal( ($.browser.msie && parseInt($.browser.version) == 6)?el.outerHeight() : el.outerHeight() , el.parent().outerHeight(), '初始化指定[height:"fit"]组件高度填充父容器');
	
	el = $("#tab-option-height-number").omTabs({height:'300px'});
...
asyncTest("lazyLoad", function() {
	...
	var elNotLazy = $("#tab-option-lazyLoad-false").omTabs({lazyLoad:false});
	var elLazy = $("#tab-option-lazyLoad-true").omTabs({lazyLoad:true});
	setTimeout(function(){
...
asyncTest("scrollable", function() {
	...
	el = $("#tab-option-scrollable-default").omTabs({width: 40});
	...
	el = $("#tab-option-scrollable-true").omTabs({width:'200',scrollable:true});
	...
	var el2 = $("#tab-option-scrollable-false").omTabs({width:'200',scrollable:false});
...
asyncTest("switchMode", function() {
	...
	el = $("#tab-option-default").omTabs();
	equal(el.omTabs("getActivated") , "tab1" , "默认初始化打开第一个页签");
	...
	el = $("#tab-option-switchMode-mouseover").omTabs({switchMode:'mouseover'});
	notEqual(el.omTabs("getActivated") , "tab2" , "初始化指定[switchMode:'mouseover']第二个页签未选中");
...
test("width", function() {
	...
	equal(($.browser.msie && parseInt($.browser.version) == 6)?el.outerWidth():el.outerWidth() , el.parent().width(), '初始化指定[width:"fit"]组件宽度填充父容器');
	
	el = $("#tab-option-width-number").omTabs({width:'300px'});
